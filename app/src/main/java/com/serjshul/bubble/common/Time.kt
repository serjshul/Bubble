package com.serjshul.bubble.common
...
fun getCreatedTimeLong(date: Date): String {
    ...
    if (itemCalendar.after(checkCalendar)) {
        ...
        return "${difference} min${if (difference <= 1) "" else "s"} ago"
    }
    ...
    if (itemCalendar.after(checkCalendar)) {
        ...
        return "${difference} hour${if (difference <= 1) "" else "s"} ago"
    }
    ...
    if (itemCalendar.after(checkCalendar)) {
        ...
        return "${difference} day${if (difference <= 1) "" else "s"} ago"
    }
    ...
    return if (itemCalendar.after(checkCalendar)) {
        ...
        "${difference / 7} week${if (difference <= 1) "" else "s"} ago"
    } else {
        val currentYear = currentCalendar.get(Calendar.YEAR)
        val itemYear = itemCalendar.get((Calendar.YEAR))
        val difference = currentYear - itemYear
        "${difference} year${if (difference <= 1) "" else "s"} ago"
    }
}

fun getCreatedTimeShort(date: Date): String {
    val itemCalendar = Calendar.getInstance()
    itemCalendar.time = date
    val currentCalendar = Calendar.getInstance()
    currentCalendar.time = Date()
